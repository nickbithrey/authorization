    ...
    private Map<String, ?> extractAdditionalInformation(String additionalInformation) {
        ...
        try {
            ...
            throw new InternalAuthenticationServiceException(
                    String.format("Could not parse additional information of oauth client [%s]", additionalInformation),
                    e);
        }
    }

    private static <T extends OAuthEnumType> Collection<String> extractTypes(Collection<T> types) {
        return types.stream().map(OAuthEnumType::extract).collect(Collectors.toSet());
    }
